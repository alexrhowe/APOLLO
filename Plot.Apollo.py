...
if datatype == "Spectrum":
    ...
    dcalmid = 10000.0 / ((dcalhi + dcallo) / 2.0)
    ...
    rcalmid = (
        10000.0 / ((rcalhi + rcallo) / 2.0) + 0.0000
    )  # Is that the wavelength offset?
    ...
    fout = open(
        "modelspectra/binned.spectrum.dat", "w"
    )  # Need a filename format. Easier as part of makespectrum.
    for i in range(0, rlen):
        fout.write(
            "{0:8.2f} {1:8.2f} {2:8.5e} {3:8.5e} {3:8.5e} {4:8.5e}\n".format(
                rcalhi[i], rcallo[i], bflux[i], bnoise[i], bflux[i]
            )
        )
    ...
    ax.plot(
        rcalmid, bflux, "-", linewidth=1, label="Retrieved Spectrum (binned)", c="b"
    )
    ...
    ax.plot(
        ...
        "-",
        linewidth=1,
        label="Residuals (binned and offset)",
        c="r",
    )
    ax.plot([xmin, xmax], [0.0, 0.0], "-", c="k")
    ax.plot([xmin, xmax], [ymin / 2.0, ymin / 2.0], "--", c="k")
    ...
    for i in range(0, len(pnames)):
        ...
        if pnames[i][0] == "T" and not pnames[i] == "Teff":
            temps.append(i)
    ...
    fig = corner.corner(
        gsamples, labels=gnames, range=grange, plot_datapoints=False, labelsize=24
    )
    ...
    fig2 = corner.corner(
        bsamples, labels=bnames, range=brange, plot_datapoints=False, labelsize=24
    )
    ...
    for i in range(0, len(temps)):
        plist[i] = 10 ** (maxP + (minP - maxP) * i / (len(temps) - 1))
    ...
    for i in range(0, len(blist[0])):
        if len(pnames[i]) <= 6:
            print(
                "{0:s}:\t\t {1:10.5f} {2:10.5f} {3:10.5f}".format(
                    pnames[i], blist[0][i], blist[1][i], blist[2][i]
                )
            )
        else:
            print(
