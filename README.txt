...
APOLLO v0.9.10rc Release Candidate
18 June 2019
...
multiple temperature-pressure profile prescriptions, and multiple priors. It
uses cgs units unless otherwise specified.

APOLLO is written in C++ and Python using the Cython extension. Anaconda 2 is
...
opaque cloud deck, and a cloud slab of uniform particle size and density.
...
Makespectrum uses the same input file format as APOLLO and generates spectrum
files that can be used as "Model"-type observations in APOLLO. Makespectrum
currently produces only JWST spectroscopic modes. However, it does output the
full-resolution spectrum to compute noise levels for photometric modes or
model pipelines for other observatories.
...
As packaged, these tables are to be read from a subdirectory named Opacities
in the code directory. If you use a different directory, you will need to
change the file path in the constructor of Atmosphere.cpp, and in readopac in
Planet_layer.cpp and Planet_auto.cpp.
...
example file, example.params.dat. If a second command line argument, "Serial",
is added, it will override parallel operation and initiate a minimal-length
serial run for testing purposes.

APOLLO may be run in serial mode from the command line for testing purposes if
parallel mode is turned off in the input file. In this configuration, it may
be run as a regular Python program:

python Apollo.py <input file>

The default mode of APOLLO is parallel processing.  On Flux, this can be done
using a PBS script incorporating the following command, which must be included
in a PBS script:

mpirun python Apollo.py <input file> >> /scratch/lsa_flux/<uniqname>/Apollo.log

The specification of the log file is optional, but is recommended to avoid
using up limited space in your home directory.
...
the parameters. The parameters must be grouped into blocks with specific
label in a specific order.

Most of the parameters have default settings. However, the input file must
contain the number of parameters listed and the parameters for the T-P
profile. The example.default.dat file shows the minimum required input file.
...
Polyn, 1 field:
1. Degree of polynomial continuum normalization. Use 0 for no normalization.
Default: 0.
...
order. Any individual block may be omitted, but at least one parameter much be
included to function. Each parameter is listed with 6 fields:
1. Parameter name.
2. Initial guess.
...
file. If not included, the T-P profile will be interpolated to 71 points.
The default if the T-P profile is omitted is an isothermal atmosphere at
1500 K.

T1-T15: an arbitrary number of temperature points in kelvin. 15 is recommended.
Use 1 for an isothermal atmosphere. The default cross section tables have
boundaries of 75 K and 4,000 K. These are also the default limits.
...
deltaL: wavelength offset to correct for calibration errors of observations.
Default: 0.0
...
not supported in v0.9.9. Use MakeHaze to compute haze opacity tables.
...
paramters of each sample in the order they appear in the input file.
...
Add RA, and Dec to the input file for Makespectrum.
