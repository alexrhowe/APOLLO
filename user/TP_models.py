...
def power_linear(exponent, logPiso, T0, T_mid, T_max,
    ...
    output_Ts = np.where((input_Ps>logPmid) & (input_Ps<logPiso),
                         ...
                         output_Ts)
    profile = np.where(input_Ps >= logPiso,
                       T_max,
                       output_Ts)
...
def sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type,
    ...
    set_bound = lambda initial, minimum, maximum: np.min([np.max([initial, minimum, minT]), maximum, maxT])
...
#################################################
def piette_monotonic_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type):
    # Assume 10 temperature nodes: log(P/bar) = [-4, -3, -2, -1, 0, 0.5, 1, 1.5, 2, 2.5]

    # logP=0.5 starts with the priors input by the user.
    logP_0p5 = 5
    prior_logP_0p5, T_logP_0p5 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_0p5, [minT, maxT])

    # logP=-4 has an upper prior bound set by logP=0.
    logP_m4 = 0
    prior_logP_m4, T_logP_m4 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_m4, [minT, T_logP_0p5])

    # logP=-2's bounds are themselves bounded by logP=-4 and logP=0.
    logP_m2 = 2
    prior_logP_m2, T_logP_m2 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_m2, [T_logP_m4, T_logP_0p5])

    # logP=-3's bounds are themselves bounded by logP=-4 and logP=-2.
    logP_m3 = 1
    prior_logP_m3, T_logP_m3 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_m3, [T_logP_m4, T_logP_m2])

    # logP=-1's bounds are themselves bounded by logP=-2 and logP=0.5.
    logP_m1 = 3
    prior_logP_m1, T_logP_m1 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_m1, [T_logP_m2, T_logP_0p5])

    # logP=0's bounds are themselves bounded by logP=-1 and logP=0.5.
    logP_0 = 4
    prior_logP_0, T_logP_0 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_0, [T_logP_m1, T_logP_0p5])

    # logP=2 has a lower prior bound set by logP=0.5.
    logP_2p5 = 9
    prior_logP_2p5, T_logP_2p5 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_2p5, [T_logP_0p5, maxT])

    # logP=1.5's bounds are themselves bounded by logP=0.5 and logP=2.5.
    logP_1p5 = 7
    prior_logP_1p5, T_logP_1p5 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_1p5, [T_logP_0p5, T_logP_2p5])

    # logP=2's bounds are themselves bounded by logP=1 and logP=2.
    logP_2 = 8
    prior_logP_2, T_logP_2 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_2, [T_logP_1p5, T_logP_2p5])

    # logP=1's bounds are themselves bounded by logP=0.5 and logP=1.5.
    logP_1 = 6
    prior_logP_1, T_logP_1 = sample_dependent_prior(distribution_functions, distribution_parameter_sets, input_values, sampler_type, logP_1, [T_logP_0p5, T_logP_1p5])

    proposed_profile = [T_logP_m4, T_logP_m3, T_logP_m2, T_logP_m1, T_logP_0,
                        T_logP_0p5, T_logP_1, T_logP_1p5, T_logP_2, T_logP_2p5]

    priors = [prior_logP_m4, prior_logP_m3, prior_logP_m2, prior_logP_m1, prior_logP_0,
              prior_logP_0p5, prior_logP_1, prior_logP_1p5, prior_logP_2, prior_logP_2p5]

    return np.array([prior.evaluate_prior_from_sample(proposed_node, sampler_type)
